...
def _simulate_transaction() -> dict[str, Any]:
    ...
    if random.random() < 0.18:
        quantity = random.randint(51, 95)
    else:
        quantity = random.randint(3, 36)
...
def dashboard() -> Any:
    ...
    payload = {
        ...
        "metrics": {
            ...
            "active_anomalies": len([a for a in ANOMALY_FEED if a["timestamp"].startswith(datetime.utcnow().strftime("%Y-%m-%d"))]),
